...
\class{binding.ActiveDescriptors} (such as \class{binding.Base}), or supply
...
not?"  It knows because there is an entry in the object's dictionary for the
value.  (Notice, by the way, that this means objects without dictionaries can't
get much use out of bindings.)
...
conjunction with the \class{binding.requireBinding} binding class, which simply
raises an error when you try to access its attribute.  It's a handy way of
specifying that a component must have the attribute, but that a subclass or
instance must supply the value.

By the way, it's important to note that bindings do \emph{not} participate in
persistence, as they directly alter the associated object's dictionary.  You
should carefully consider the use of attribute bindings in persistent objects,
as to how they will interact with your persistence machinery (e.g. whether they
should be saved or not) and whether you will want to write them in such a way
as to consider the object ``changed" when an attribute is computed.





\subsection{Available Binding Types}

XXX AutoCreated, bindSequence, bindTo, requireBinding, bindToParent, 
bindToProperty, bindToUtilities, Constant, Acquire, bindToSelf, Copy, New,
Once, OnceClass
