...
derive from \class{binding.Base}, though, like \class{binding.Component}.
You'll likely use \class{binding.Component} a lot when defining
...
tell it.  The constructor for \class{binding.Base} takes as its very first
...
Python built-in types, are always root components.  For example:
...
/
>>> print binding.lookupComponent("upper", "another string")
...
Notice also that an object doesn't have to be based on \class{binding.Base}
...
\class{binding.IBindingSPI}, that defines what methods are used by the binding
framework.  SPI stands for \strong{Service Provider Interface}, which is kind
of like the opposite of an API.  APIs are provided \emph{by} frameworks to
applications, while SPIs define what a framework expects an application or
plug-in to provide \emph{to} the framework.  Anyway, the \class{binding.%
IBindingSPI} interface looks basically like this:

\begin{verbatim%
}class IBindingSPI(config.IConfigSource):
...
So, the \class{binding.IBindingSPI} interface tells us that to implement the
Binding SPI, we need a \function{getParentComponent} method and a \function{%
getComponentName} method, neither of which takes any parameters, and whose
return values are as documented.
...
some additional methods which a Binding SPI component should implement.  We'll
...
\class{binding.IBindingSPI} interface is also promising to implement the
\class{config.IConfigSource} interface as well.

Did our example \class{myGUIControl} class promise to implement \class{%
binding.IBindingSPI}?  No.  It doesn't have an \samp{__implements__} attribute
or inherit one, so it's not promising to implement the full \class{IBindingSPI}
...
component implement all of the \class{IBindingSPI} methods.  Many other
interfaces however, \emph{must} be promised by a class in order for the
functionality to work.

The \class{binding.IBindingFactory} interface is a good example of this.
\class{IBindingFactory} is the interface which defines how component 
...
arguments they took, or how it was that the objects in our examples wound up
with parents when we never supplied any arguments to any constructors.  Well,
we're about to see how that works.

When a \function{binding.New} binding is activated, it checks to see if
its \var{obtype} parameter implements \class{IBindingFactory}.  If so,
...
doesn't implement \class{IBindingFactory}, it's simply called without any
...
Let's take a look at the \class{binding.IBindingFactory} interface in more
detail.
...
instances or in your class.  It simply means that an object which implements
the interface should be callable.  Because \class{IBindingFactory} is an
...
XXX IBindingAPI interface;  setParentComponent
...
XXX Base, Component, bindToParent, ...
