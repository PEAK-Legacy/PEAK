...
web-servers, for example.  We have found the clusterit tools
(http://www.garbled.net/clusterit.html) to be very useful in such
situations.  Clusterit, inspired by the tools provided in IBM's PSPP, is
simple to set up and use, if somewhat quirky in implementation.
...
of these properties in your application hasn't set them up.  In the
abscence of a cluster definition file, PEAK will behave as if the local
hostname was listed in the cluster file (as a member of _orphans/_all). 
...
memberships of the first four groups, odd, even, prime, and qux, should
be clear.  A "lump" is a group defined in terms of other groups or
...
peak.running.cluster._host.one.baz.com is 'odd', 'prime', 'qux', 'wierd'.
...
def loadCluster(propertyName, prefix, targetObj=None):
    ...
    if propertyName == prefix + "_filename":
        return config.getProperty('environ.CLUSTER', targetObj, default=None)
